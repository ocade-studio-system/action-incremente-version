...
const getFileContent = pathFile => {
	try {
		...
		throw new Error(`🆘 Impossible de lire le fichier: ${pathFile}`);
	}
};
...
const toKebabCase = string => {
	return string.replaceAll(" ", "_").toLowerCase().replace(/^_/, "").replace(/^\*_/, "");
};
...
const extractComment = fileContent => {
  let regexPlugin = "^<\\?php\\n\\n?\\/\\*\\*\\n(((\\s)*?\\*\\s.+):\\s?(.+\\n))+\\s?\\*\\/";
	let regexTheme = "^\\/\\*\\*?\\n((\\s)*?\\*\\s.+:\\s?.+\\n)+\\*\\/";
	let regexp = isContentBegin(fileContent, "<?php") ? regexPlugin : regexTheme;
...
const extractVersionComment = indexFile => {
	...
	const regexp = new RegExp("Version(\\s)?:(.*)", "g");
	const matches = [...comment.matchAll(regexp)];
	for (const match of matches) {
		return incrementeVersion(match[2].trim());
	}
};
...
const extractVersion = indexFile => {
	return indexFile === "./package.json"
		? extractVersionPackageJson(indexFile)
		: extractVersionComment(indexFile);
};
